...
public class E2E_HashMap_DataProvider extends base.BaseTest{
	
 
    
	@Test(dataProvider = "getEData", dataProviderClass = E2EData.class)
	public void endToend(HashMap<String, String> hm) throws InterruptedException, IOException
	{
	    // Home Page check
	    ...
	    // Use HashMap data
	    lp.enterNameEmail(hm.get("name"), hm.get("email"));
	    SignUp_Page sp = lp.clickSignUp();

	    Assert.assertTrue(sp.isAccountInfoPageVisible(), "Account information page not opened");

	    // Prefilled checks
	    Assert.assertEquals(sp.getPrefilledName(), hm.get("name"), "Name not prefilled correctly");
	    Assert.assertEquals(sp.getPrefilledEmail(), hm.get("email"), "Email not prefilled correctly");

	    // Fill account details using HashMap
	    ...
	    Assert.assertTrue(hp.isUserLoggedIn(hm.get("name")), "User not logged in correctly");
	    ...
	    for (int i = 0; i < productsToAdd.size(); i++) {
	        ...
	        if (i == productsToAdd.size() - 1) {
	            cp = pp.clickViewCart(); // last product
	        } else {
	            pp.clickContinueShopping(); // others
	        }
	    }
